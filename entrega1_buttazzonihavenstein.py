...
df_limpio["ACCOUNT_STATUS"].value_counts()
...
sns.boxplot(df_limpio["MONTHS_OF_BALANCE"], orient="V")
...
sns.FacetGrid(df, hue="GENDER", size= 5).map(plt.scatter,"ACCOUNT_STATUS", "INCOME_CATEGORY").add_legend();
plt.show
...
creditcard_tree = DecisionTreeClassifier(max_depth=2, random_state = 42)
...
modelos = [creditcard_tree, randomf]
...
#Vemos que para nuestro dataset, tenemos mayor accuracy con RandomForest
...
df2 = df_limpio.loc[:,"ANNUAL_INCOME"]
df3= df_limpio.loc[:,"ACCOUNT_STATUS"]

df_limpio

hdbscan = HDBSCAN(min_cluster_size=20,min_samples=10)
pred= hdbscan.fit_predict(df_limpio)
...
df_limpio.plot(kind='scatter', x='ACCOUNT_STATUS', y='ANNUAL_INCOME', c=pred, cmap='Accent_r', figsize=(16,10))

# nuestro modelo debe ser de aprendizaje no supervisado